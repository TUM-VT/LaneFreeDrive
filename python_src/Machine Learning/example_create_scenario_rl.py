...
experiments_name = "example_rl_study"

# Path of the sumo config file to be used for the experiment. Change it for a different SUMO configuration.
sumo_config_file = Path().absolute().parent.joinpath("SUMO_Runs\\example_ring_road\\ring.sumocfg")
...
default_ini_path = Path().absolute().parent.parent.joinpath("src", "lft_plugin_config_files", "default_config.ini")
...
# file.
params = {"RANDOM Init:vehicle_types": ["AV_1,AV_2,Human_1,Human_2"],
          "RANDOM Init:vehicle_counts": [(30,30,30,30)],
          ...
          "Adaptive Potential Lines Parameters:AdaptiveAlgorithm": ["ConstantMargin", "PythonRL"],
...
for _, row in df.iterrows():
    ...
    margin_file = str(parent_folder.joinpath(scenario, "python_margin_rl.csv"))
    d_config["Adaptive Potential Lines Parameters"]["PythonMarginInputFile"] = margin_file
